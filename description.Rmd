...
The majority of the data is less than $50, though there is a long tail.
...
We see a lot of rides around midnight and a consistent number of rides throughout the midday (10am-4pm).
...
When we look at Uber and Lyft seperately, we see that Uber consistently has more rides at each hour than Lyft.
...
The same holds true for the number of rides per day.
...
As above, Uber has more rides than Lyft to and from every location. However, the number of rides associated with each location are fairly consistent.
...
Interestingly, the most rides correspond to the overcast and cloudy days and times. I would have expected rain to have a higher proportion of the data.
...
The surge multiplier is always 1 for Uber. There's no clear correlation for the Lyft plot. 
...
Looking at the apparent temperature variables is quite difficult given the amount of data. However, there dosn't seem to be much of a correlation.
...
There's quite a bit of red and blue (meaning strong positive and negative correlations). However, many of the variables in our data set are associated with each other (ex: weather and temperature) so it isn't surprisng to see a plot like this.
